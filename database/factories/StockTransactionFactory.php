...
class StockTransactionFactory extends Factory
{
    ...
    private function generateReferenceNumber(string $transactionType): string
    {
        return match($transactionType) {
            'procurement' => 'PO-' . $this->faker->dateFormat('Y') . '-' . $this->faker->numberBetween(1000, 9999),
            'sale' => 'POS-' . $this->faker->dateFormat('Ymd') . '-' . $this->faker->numberBetween(100, 999),
            'reservation' => 'JO-' . $this->faker->dateFormat('Y') . '-' . $this->faker->numberBetween(1000, 9999),
            'return' => 'RET-' . $this->faker->dateFormat('Ymd') . '-' . $this->faker->numberBetween(100, 999),
            'damage' => 'DMG-' . $this->faker->dateFormat('Ymd') . '-' . $this->faker->numberBetween(100, 999),
            'adjustment' => 'ADJ-' . $this->faker->dateFormat('Ymd') . '-' . $this->faker->numberBetween(100, 999),
            default => 'TXN-' . $this->faker->dateFormat('Ymd') . '-' . $this->faker->numberBetween(100, 999),
        };
    }
    ...
    public function procurement(): static
    {
        return $this->state(fn (array $attributes) => [
            ...
            'reference_number' => 'PO-' . $this->faker->dateFormat('Y') . '-' . $this->faker->numberBetween(1000, 9999),
        ]);
    }
    ...
    public function sale(): static
    {
        return $this->state(fn (array $attributes) => [
            ...
            'reference_number' => 'POS-' . $this->faker->dateFormat('Ymd') . '-' . $this->faker->numberBetween(100, 999),
        ]);
    }
}
