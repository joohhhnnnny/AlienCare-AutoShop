...
class AlertService {
  ...
  async getAlerts(filters: AlertFilters = {}): Promise<ApiResponse<PaginatedResponse<Alert>>> {
    ...
    return api.get<ApiResponse<PaginatedResponse<Alert>>>('/alerts', params);
  }

  // Get alert statistics and summary
  async getAlertStatistics(): Promise<ApiResponse<AlertStatistics>> {
    return api.get<ApiResponse<AlertStatistics>>('/alerts/statistics');
  }

  // Generate low stock alerts
  async generateLowStockAlerts(): Promise<ApiResponse<{
  ...
  async acknowledgeAlert(alertId: number): Promise<ApiResponse<Alert>> {
    return api.put<ApiResponse<Alert>>(`/alerts/${alertId}/acknowledge`);
  }

  // Bulk acknowledge multiple alerts
  async bulkAcknowledgeAlerts(alertIds: number[]): Promise<ApiResponse<{
    acknowledged_count: number;
  }>> {
    return api.post<ApiResponse<{
      acknowledged_count: number;
    }>>('/alerts/bulk-acknowledge', { alert_ids: alertIds });
  }

  // Cleanup old acknowledged alerts
  async cleanupAlerts(days: number = 30): Promise<ApiResponse<{
    deleted_count: number;
  }>> {
    return api.delete<ApiResponse<{
      deleted_count: number;
    }>>(`/alerts/cleanup?days=${days}`);
  }
